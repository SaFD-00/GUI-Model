...
| 데이터셋 | 이미지 수 | 경로 패턴 |
|----------|----------|----------|
| MobiBench | 3,655개 | `MobiBench/images/episode_{id:06d}_step_{idx:04d}.png` |
| AndroidControl | 추출 필요 (`scripts/extract_androidcontrol_images.py`) | `AndroidControl/images/episode_{id:06d}_step_{idx:04d}.png` |
...
| num_train_epochs | 5 | 3 | MB는 gWorld/MobileDreamer를 따라 5 epochs, AC는 데이터 규모(~67K)에 맞춰 3 epochs로 수렴 여유 확보 (post-training Hungarian F1 sweep 이 과적합 epoch 을 자동 회피) |
| weight_decay | 0.01 | 0.01 | gWorld 동일, 표준 정규화 |
| max_grad_norm | 1.0 | 1.0 | gradient explosion 방지 |
| save_strategy | epoch | epoch | 2026-04-13 재검토 시 AC=steps(500) 이었으나 관리 일관성 위해 MB와 동일 epoch 로 통일 (3 epoch = checkpoint 3개, 디스크 부담 경감) |
| save_total_limit | 5 | 5 | 디스크 사용량 제한 (epoch=3 이므로 실제로는 3개만 생성) |
| 학습 중 eval | **비활성화** | **비활성화** | 학습 YAML 에 eval 필드 없음. 학습 시간/VRAM 절약. Best checkpoint 는 post-training `stage1_eval.sh` 의 Hungarian F1 sweep 이 선택 |
...
| num_train_epochs | 5 | 3 | MB는 5 epochs, AC는 Stage 1과 대칭으로 3 epochs (post-training Overall Score sweep 이 과적합 epoch 을 자동 회피) |
| weight_decay | 0.01 | 0.01 | 표준 |
| max_grad_norm | 1.0 | 1.0 | gradient explosion 방지 |
| save_strategy | epoch | epoch | S1과 동일한 정책 (2026-04-13 steps→epoch 통일) |
| save_total_limit | 5 | 5 | 디스크 사용량 제한 (epoch=3 이므로 실제 3개) |
| 학습 중 eval | **비활성화** | **비활성화** | 학습 YAML 에 eval 필드 없음. Best checkpoint 는 post-training `stage2_eval.sh` 의 Overall Score sweep 이 variant 별 (`lora_base` / `lora_world_model`) 독립 선택 |
...
> `gui-model.ipynb` Section 0 의 **Stage 1 Training YAML 블록(Cell 8)** 이 `CONFIGS[ds_name]["stage1"]` 기반으로 동적 생성. 아래는 MobiBench 예시이며 AndroidControl은 데이터셋 접두사/출력 경로/하이퍼파라미터가 7.2 표와 같이 분기된다.
...
logging_steps: 1
save_strategy: epoch           # AC: epoch (2026-04-13 steps(500)→epoch 통일)
save_total_limit: 5
plot_loss: true
...
### 8.2 Stage 1 평가 (post-training): `stage1_eval/eval_loss.yaml`
...
### output
output_dir: ./saves/MB/stage1_eval/hungarian_matching/full_world_model
overwrite_output_dir: true
...
> `gui-model.ipynb` Section 0 의 **Stage 2 Training YAML 블록(Cell 14)** 이 데이터셋별로 `base.yaml`과 `world_model.yaml` 두 개를 자동 생성. 두 파일은 `model_name_or_path`와 `output_dir`만 다르다.
...
**등록**: Cell 19/22 실행 시 `LlamaFactory/data/dataset_info.json`에 상대 경로로 자동 등록:
...
전체 파이프라인은 `gui-model.ipynb` 셀 실행이 표준이지만, Section 0 (Cell 3–16, YAML Configs 일괄 생성 포함) 과 데이터 등록 셀(Cell 18/21) 을 한 번 실행하면 이후 학습/평가/Merge 단계는 `scripts/` 쉘 스크립트로 반복 실행할 수 있다.

### 9.0 쉘 스크립트 (권장, 재실행·자동화)

| 스크립트 | 대응 Cell | 수행 |
|---|---|---|
| `scripts/stage1_train.sh` | 25 | Stage 1 Full FT (FORCE_TORCHRUN=1, H100×4) → `saves/{DS}/stage1_full/full_world_model/` |
| `scripts/stage1_eval.sh`  | Section 4 cells | **Phase A** Baseline(Zero-shot) Hungarian → **Phase B** 전체 checkpoint sweep(vllm_infer + `_hungarian_eval.py score`) → **Phase C** winner 선택(`_hungarian_eval.py select`) → `saves/{DS}/.../BEST_CHECKPOINT` 파일 기록 |
| `scripts/stage1_merge.sh` | 12 (YAML 재실행) + Section 5 cell | **BEST_CHECKPOINT 필수(없으면 hard-fail)**. Section 0 의 **Cell 12** 재실행으로 winner 경로가 YAML 에 반영됨 → `llamafactory-cli export` (HF Hub push) → `exports/.../` 를 `outputs/{DS}/stage1_merged/` 로 rsync |
| `scripts/stage2_train.sh` | 38+39 | Stage 2 LoRA (base, world_model) → `saves/{DS}/stage2_lora/{lora_base,lora_world_model}/` |
| `scripts/stage2_eval.sh`  | Section 7 cells | **Phase A** Baseline(Zero-shot) → **Phase B** `lora_base`/`lora_world_model` 각각 체크포인트 sweep (`--adapter_name_or_path`) + `_action_eval.py score` → **Phase C** winner 선택(`select`, 기본 지표 `overall_score`) → 각 lora 디렉토리에 `BEST_CHECKPOINT` 기록. **완전 로컬 — HF Hub 의존 없음** |
| `scripts/stage2_merge.sh` | 16 (YAML 재실행) + Section 8 cells | **각 variant BEST_CHECKPOINT 필수(없으면 hard-fail)**. Section 0 의 **Cell 16** 재실행으로 winner adapter 경로가 YAML 에 반영됨 → `llamafactory-cli export` (HF Hub push) → `outputs/{DS}/stage2_merged/{base,world_model}/` 로 rsync. `lora_world_model` 의 base 는 `outputs/{DS}/stage1_merged` (로컬) |

**Best Epoch 자동 선택** (Stage 1/2 공통 패턴, 지표만 다름):
- Stage 1: `stage1_eval.sh` → `_hungarian_eval.py` → Hungarian F1 winner → `saves/{DS}/stage1_full/full_world_model/BEST_CHECKPOINT`
- Stage 2: `stage2_eval.sh` → `_action_eval.py` → Overall Score winner → `saves/{DS}/stage2_lora/{lora_base,lora_world_model}/BEST_CHECKPOINT` (각 variant 독립 선택)
- Merge 스크립트/cell 은 BEST_CHECKPOINT 없을 시 **hard-fail** (fallback 없음) — eval 누락을 조기 감지

공통 인자: `MB | AC | all` (기본 `all`). 로그: `logs/<script>_<DS>_<ts>.log` 로 tee. `set -euo pipefail` 로 단계 실패 시 즉시 중단. 요구사항: bash 4+, LLaMA-Factory 설치, merge 스크립트는 `.env` 의 `HF_TOKEN` 필요.
...
# 전체 파이프라인 (6-스크립트 순서, one-shot Hungarian 선택)
...
./scripts/stage2_train.sh     # 4. Stage 2 LoRA (HF 모델 base)
./scripts/stage2_eval.sh      # 5. 3-Way prediction
./scripts/stage2_merge.sh     # 6. Stage 2 merge & push
...
# Stage 2: Action Prediction LoRA FT
# stage2, stage1+stage2 각각 별도 YAML (노트북 Cell 38/39 에는 torchrun prefix 없음)
...
# Stage 2 평가: 동일한 vllm_infer.py 를 3-Way (base / lora_base / lora_world_model) 실행
# gui-model.ipynb Section 8 참조
```

### 9.3 모델 Merge & 배포 (수동, 디버깅용)
...
| Stage 1 평가 리포트 | `saves/{DS}/stage1_eval/*/evaluation_report.md` | Loss + Hungarian 메트릭 | ✅ |
| Stage 2 평가 리포트 | `saves/{DS}/stage2_eval/*/evaluation_report.md` | 모델별 상세 메트릭 | ✅ |
| 시각화 차트 | `saves/{DS}/stage2_eval/stage2_evaluation.png` | 3-Way 비교 차트 | ✅ |
...
│   ├── _common.sh                  # 쉘 스크립트 공통 헬퍼
│   ├── _hungarian_eval.py          # Stage 1 체크포인트별 Hungarian/BLEU/ROUGE + winner 선택 (score/select 서브커맨드)
│   ├── stage1_train.sh             # Stage 1 Full FT (Cell 25)
│   ├── stage1_eval.sh              # Stage 1 평가 — Baseline + checkpoint sweep → Hungarian F1 winner 선택
│   ├── stage1_merge.sh             # Stage 1 Merge — BEST_CHECKPOINT 기반 HF push + 로컬 복사
│   ├── stage2_train.sh             # Stage 2 LoRA (Cell 38+39)
│   ├── stage2_eval.sh              # Stage 2 3-Way 평가 (Cell 44+45+46)
│   └── stage2_merge.sh             # Stage 2 Merge & HF Upload (Cell 41+42)
│
...
│   │       └── stage1_eval/        # Stage 1 평가 설정 (legacy, Cell 10; shell 파이프라인은 미사용)
│   ├── saves/                      # 학습 중간 결과 (training checkpoints + post-training eval)
│   │   ├── {DS}/stage1_full/
│   │   │   └── full_world_model/   # checkpoint-* (epoch 별 저장)
│   │   │       ├── BEST_CHECKPOINT         # Hungarian F1 winner 이름 (plain text, stage1_eval.sh 가 기록)
│   │   │       └── BEST_CHECKPOINT.json    # winner 선정 상세(후보 랭킹, metric 값)
│   │   ├── {DS}/stage1_eval/       # Stage 1 평가 중간 결과
│   │   │   ├── eval_loss/          # Loss 메트릭 (base / full_world_model)
│   │   │   └── hungarian_matching/ # Hungarian 메트릭
│   │   │       ├── base/           # Zero-shot 비교
│   │   │       └── checkpoint-*/   # 체크포인트별 Hungarian 스코어(hungarian_metrics.json)
│   │   ├── {DS}/stage2_lora/
│   │   │   ├── lora_base/          # Stage 2 LoRA base adapter (checkpoint-* + BEST_CHECKPOINT)
│   │   │   └── lora_world_model/   # Stage 2 LoRA world_model adapter (checkpoint-* + BEST_CHECKPOINT)
...
│   │   ├── {DS}/stage1_merged/     # Stage 1 winner merge 로컬 복사 (exports/ 의 rsync)
│   │   └── {DS}/stage2_merged/
│   │       ├── base/               # Stage 2 merge_base winner merge 복사
│   │       └── world_model/        # Stage 2 merge_world_model winner merge 복사
│   └── data/                       # 데이터 설정 템플릿
