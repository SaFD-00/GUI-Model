...
전체 파이프라인은 `gui-model.ipynb` 셀 실행이 표준이지만, YAML/데이터 등록 셀(Cell 3/7/10/14/17/20/30/34)을 한 번 실행하면 이후 학습/평가/Merge 단계는 `scripts/` 쉘 스크립트로 반복 실행할 수 있다.

### 9.0 쉘 스크립트 (권장, 재실행·자동화)

| 스크립트 | 대응 Cell | 수행 |
|---|---|---|
| `scripts/stage1_train.sh` | 15 | Stage 1 Full FT (FORCE_TORCHRUN=1, H100×4) + `load_best_model_at_end=true` safety net |
| `scripts/stage1_eval.sh`  | 23+24+26 | **Phase A** Baseline(Zero-shot) Hungarian → **Phase B** 전체 checkpoint sweep(vllm_infer + `_hungarian_eval.py score`) → **Phase C** winner 선택(`_hungarian_eval.py select`) → `BEST_CHECKPOINT` 파일 기록 |
| `scripts/stage1_merge.sh` | 17+18 | `BEST_CHECKPOINT` 읽어 해당 체크포인트 경로로 merge YAML 런타임 override → `llamafactory-cli export` (HF Hub push) → `exports/.../` 를 `outputs/{DS}/stage1_merged/` 로 rsync (로컬 복사) |
| `scripts/stage2_train.sh` | 31+32 | Stage 2 LoRA (base, world_model) + checkpoint-* 생성 + `load_best_model_at_end` safety net |
| `scripts/stage2_eval.sh`  | 38+39+40+42 | **Phase A** Baseline(Zero-shot) → **Phase B** `lora_base`/`lora_world_model` 각각 체크포인트 sweep (`--adapter_name_or_path`) + `_action_eval.py score` → **Phase C** winner 선택(`select`, 기본 지표 `overall_score`) → 각 lora 디렉토리에 `BEST_CHECKPOINT` 기록. **완전 로컬 — HF Hub 의존 없음** |
| `scripts/stage2_merge.sh` | 34+35+36 | 각 variant `BEST_CHECKPOINT` 읽어 merge YAML 의 `adapter_name_or_path` + `model_name_or_path` 런타임 override → `llamafactory-cli export` (HF Hub push). `lora_world_model` 의 base 는 `outputs/{DS}/stage1_merged` (로컬) |
...
# stage2, stage1+stage2 각각 별도 YAML (노트북 Cell 31/32 에는 torchrun prefix 없음)
...
│   │       └── stage1_eval/        # Stage 1 평가 설정 (legacy, Cell 20; shell 파이프라인은 미사용)
│   ├── outputs/                    # 학습 및 평가 결과
│   │   ├── {DS}/stage1_full/
│   │   │   └── full_world_model/   # checkpoint-* + load_best_model_at_end 결과
│   │   │       ├── BEST_CHECKPOINT         # Hungarian F1 winner 이름 (plain text, stage1_eval.sh/Cell 26 이 기록)
│   │   │       └── BEST_CHECKPOINT.json    # winner 선정 상세(후보 랭킹, metric 값)
│   │   ├── {DS}/stage1_eval/       # Stage 1 평가 리포트
│   │   │   ├── eval_loss/          # Loss 메트릭 (optional, Cell 21 실행 시)
