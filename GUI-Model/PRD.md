...
| `scripts/stage2_train.sh` | 31+32 | Stage 2 LoRA (base, world_model) — 노트북 원본 기준 torchrun prefix 미사용 |
| `scripts/stage2_eval.sh`  | 38+39+40 | 3-Way prediction (vllm_infer.py) |
| `scripts/stage2_merge.sh` | 35+36 | Stage 2 LoRA merge_base / merge_world_model |

**Best Epoch 자동 선택** (Stage 1 전용): `stage1_eval.sh` 가 각 `checkpoint-*/` 에 대해 vllm_infer 로 XML 생성 → `_hungarian_eval.py` 로 Hungarian F1 계산 → 최고값을 `outputs/{DS}/stage1_full/full_world_model/BEST_CHECKPOINT` 에 기록. `stage1_merge.sh` 는 이 파일을 읽어 해당 체크포인트를 merge. 파일 없으면 `load_best_model_at_end` 루트(= eval_loss winner)로 fallback.
