...
| eval_strategy | epoch | epoch | `load_best_model_at_end`의 strategy 일치 제약 충족 (save/eval 동기) |
...
| eval_strategy | epoch | epoch | `load_best_model_at_end` 제약 충족 |
...
eval_strategy: epoch            # AC: epoch (save_strategy와 동기 유지)
...
| 스크립트 | 대응 Cell | 수행 |
|---|---|---|
| `scripts/stage1_train.sh` | 25 | Stage 1 Full FT (FORCE_TORCHRUN=1, H100×4) + `load_best_model_at_end=true` safety net |
| `scripts/stage1_eval.sh`  | 31+32+34 | **Phase A** Baseline(Zero-shot) Hungarian → **Phase B** 전체 checkpoint sweep(vllm_infer + `_hungarian_eval.py score`) → **Phase C** winner 선택(`_hungarian_eval.py select`) → `BEST_CHECKPOINT` 파일 기록 |
| `scripts/stage1_merge.sh` | 12 (YAML) + 27 | Section 0 의 **Stage 1 Merge YAML 블록(Cell 12)** 재실행으로 `BEST_CHECKPOINT` 기반 체크포인트 경로가 YAML 에 반영됨 → `llamafactory-cli export` (HF Hub push) → `exports/.../` 를 `outputs/{DS}/stage1_merged/` 로 rsync (로컬 복사) |
| `scripts/stage2_train.sh` | 38+39 | Stage 2 LoRA (base, world_model) + checkpoint-* 생성 + `load_best_model_at_end` safety net |
| `scripts/stage2_eval.sh`  | 44+45+46+48 | **Phase A** Baseline(Zero-shot) → **Phase B** `lora_base`/`lora_world_model` 각각 체크포인트 sweep (`--adapter_name_or_path`) + `_action_eval.py score` → **Phase C** winner 선택(`select`, 기본 지표 `overall_score`) → 각 lora 디렉토리에 `BEST_CHECKPOINT` 기록. **완전 로컬 — HF Hub 의존 없음** |
| `scripts/stage2_merge.sh` | 16 (YAML) + 41+42 | Section 0 의 **Stage 2 Merge YAML 블록(Cell 16)** 재실행으로 각 variant `BEST_CHECKPOINT` 기반 `adapter_name_or_path` + `model_name_or_path` 가 YAML 에 반영됨 → `llamafactory-cli export` (HF Hub push). `lora_world_model` 의 base 는 `outputs/{DS}/stage1_merged` (로컬) |

**Best Epoch 자동 선택** (Stage 1/2 공통 패턴, 지표만 다름):
- Stage 1: `stage1_eval.sh` → `_hungarian_eval.py` → Hungarian F1 winner → `outputs/{DS}/stage1_full/full_world_model/BEST_CHECKPOINT`
- Stage 2: `stage2_eval.sh` → `_action_eval.py` → Overall Score winner → `outputs/{DS}/stage2_lora/{lora_base,lora_world_model}/BEST_CHECKPOINT` (각 variant 독립 선택)
- Merge 스크립트는 BEST_CHECKPOINT 없을 시 `load_best_model_at_end` 루트(=eval_loss winner)로 fallback
...
│   ├── outputs/                    # 학습 및 평가 결과
│   │   ├── {DS}/stage1_full/
│   │   │   └── full_world_model/   # checkpoint-* + load_best_model_at_end 결과
│   │   │       ├── BEST_CHECKPOINT         # Hungarian F1 winner 이름 (plain text, stage1_eval.sh/Cell 34 가 기록)
│   │   │       └── BEST_CHECKPOINT.json    # winner 선정 상세(후보 랭킹, metric 값)
│   │   ├── {DS}/stage1_eval/       # Stage 1 평가 리포트
│   │   │   ├── eval_loss/          # Loss 메트릭 (optional, Cell 29 실행 시)
...
│   │   └── {DS}/stage2_eval/       # Stage 2 3-Way 평가
