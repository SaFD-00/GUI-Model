...
| 파라미터 | MB | AC | 근거 |
|----------|-----|-----|------|
| Epochs | 5 | 3 | MB는 gWorld/MobileDreamer 관례(5 epochs), AC는 데이터 규모에 맞춰 3 epochs로 수렴 여유 확보 (post-training sweep 이 과적합 epoch 을 자동 회피) |
...
| save strategy | epoch | epoch | 2026-04-13 재검토 시 AC=steps(500) 이었으나 관리 일관성 위해 MB와 동일하게 epoch 로 통일 (checkpoint 3개만 생성 → 디스크 부담 경감) |
| 학습 중 eval | 비활성화 | 비활성화 | Post-training checkpoint sweep (`stage{1,2}_eval.sh`) 이 Hungarian F1 / Overall Score winner 를 선택 — 학습 시 VRAM/시간 절약 |
| Vision Encoder | Frozen | Frozen | Code2World, gWorld, MobileDreamer 모두 동결 |

**공통 안정화 옵션**: `weight_decay=0.01`(gWorld 정렬), `max_grad_norm=1.0`, `save_total_limit=5` — 학습 불안정/OOM 회복 보장. 학습 중 eval 은 비활성화되어 있으며, best checkpoint 는 학습 완료 후 `stage{1,2}_eval.sh` 가 `*_stage{n}_test` 데이터셋으로 checkpoint sweep 을 수행하여 Hungarian F1 (Stage 1) / Overall Score (Stage 2) 기준으로 선택합니다.
