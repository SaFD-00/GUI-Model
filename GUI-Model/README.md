...
| Learning Rate | 1e-5 (cosine) | **2e-5** (cosine) |
...
| Learning Rate | 3e-5 (cosine, warmup=0.05) | **1e-5** (cosine, warmup=0.05) |
| Epochs | 5 | **2** |
| max_grad_norm | 1.0 | 1.0 |
| save / eval strategy | epoch | steps (500) |
| save_total_limit | 5 | 5 |
...
학습 설정은 아래 관련 연구의 training recipe를 참고하되, MB(소규모, ~3K)와 AC(대규모, ~35K+)의 **데이터 규모 차이**에 맞춰 분기했습니다:

| 파라미터 | MB | AC | 근거 |
|----------|-----|-----|------|
| Epochs | 5 | 2 | MB는 gWorld/MobileDreamer 관례(5 epochs), AC는 데이터 규모로 2 epochs면 충분 |
| S1 Learning Rate | 1e-5 | 2e-5 | MB 저 LR × 다 epoch vs AC 상대적 고 LR × 적 epoch |
| S2 Learning Rate | 3e-5 | 1e-5 | MobileDreamer LoRA 참고 |
| Warmup (S1) | 0.05 | 0.1 | AC는 높은 LR에 대응해 warmup 비율 ↑ |
...
**공통 안정화 옵션**: `max_grad_norm=1.0`, `save_total_limit=5`, `load_best_model_at_end=true`(eval_loss 최소) — 학습 불안정/OOM 회복과 best checkpoint 자동 선택 보장. Eval은 별도 validation split을 만들지 않고 기존 `*_stage{n}_test` 데이터셋을 `eval_dataset`으로 재사용합니다.
