...
| `scripts/stage1_eval.sh`  | Cell 23+24+26 | **Baseline + 전체 체크포인트 sweep(vllm_infer) → `_hungarian_eval.py score` 로 Hungarian 메트릭 계산 → `select` 로 winner 자동 선택 → `BEST_CHECKPOINT` 파일 기록** |
| `scripts/stage1_merge.sh` | Cell 17+18 | `BEST_CHECKPOINT` 읽어 해당 체크포인트를 merge → HF Hub push + `outputs/{DS}/stage1_merged/` 로컬 복사 |
...
**Best Epoch 자동 선택 파이프라인** (Stage 1 고유):
```
stage1_train.sh      stage1_eval.sh                       stage1_merge.sh
  ┌─────────┐          ┌──────────────────────┐              ┌─────────────────┐
  │ ckpt-N/ │ ────┐    │ Phase A: Baseline    │              │ BEST_CHECKPOINT │
  │ ckpt-M/ │     ├──▶ │ Phase B: sweep ckpt-*│ ─── F1 ───▶ │  읽기           │
  │ ckpt-K/ │ ────┘    │ Phase C: winner 선택 │              │  → winner merge │
  └─────────┘          └──────────────────────┘              │  → HF + local   │
                                │                            └─────────────────┘
...
│   ├── _hungarian_eval.py               # Stage 1 체크포인트별 Hungarian/BLEU/ROUGE 계산 + winner 선택 (score/select 서브커맨드)
