...
> Section 0 Cell 4 는 `pip install -e .`, Cell 5 는 `LlamaFactory/` clone + editable install 흐름입니다. 위 명령과 노트북 셀은 동일한 환경 구성을 목표로 합니다.
...
1. **Section 0 — Environment Setup & YAML Configs**
   ...
   - Cell 5: `LlamaFactory/` clone + `pip install -e "./LlamaFactory[torch,metrics]"`
...
환경 설치는 위 1번 명령 또는 노트북 Section 0 Cell 4/5 로 먼저 끝낸 뒤, 노트북 Section 0 (환경 설정 + Training/Eval YAML 생성) 과 Section 1/2 (데이터 등록) 을 **한 번 실행**해 YAML · `dataset_info.json` 이 이미 존재한다는 전제하에, 학습/평가/Merge 단계는 `scripts/` 아래 쉘 스크립트로 반복 실행합니다.
