...
- [`scripts/stage1_eval.sh`](./scripts/stage1_eval.sh)
  - Phase A (baseline zero-shot, mode 무관) + Phase B (각 ckpt 의 epoch 을 parse 해 **HF Hub merged repo sweep**).
    - `vllm_infer.py --model_name_or_path <HF repo id>` 만 전달 (merged 이므로 adapter 인자 · `max_lora_rank` 불필요; full/lora 공통).
...
- [`scripts/stage2_eval.sh`](./scripts/stage2_eval.sh)
  - Phase A (baseline zero-shot, mode 무관) + Phase B (두 variant 각각 **HF Hub merged repo sweep**, backend 독립).
    - `lora_base`: `SaFD-00/...stage2-base-epoch{E}` (mode 무관)
    - `lora_world_model_from_${MODE}`: `SaFD-00/...stage2-${MODE}-world-model-epoch{E}`
    - merged 이므로 `--adapter_name_or_path` · `max_lora_rank` 전부 제거.
...
bash scripts/stage1_eval.sh  --model qwen3-vl-8b --dataset MB                        # HF sweep + winner
