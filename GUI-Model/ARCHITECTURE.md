...
- [`scripts/stage1_eval.sh`](./scripts/stage1_eval.sh)
  - 표준 실행 순서: **train → eval → merge**. eval 은 로컬 per-epoch checkpoint 만 소비하며 HF Hub 의존이 없다.
  - Phase A (baseline zero-shot, mode 무관) + Phase B (`outputs/{DS}/adapters/{MODEL}_stage1_${MODE}/checkpoint-*` per-epoch sweep, backend 독립).
  ...
  - 결과 경로: `outputs/{DS}/eval/{MODEL}/stage1_eval/{base, ${MODE}_world_model/checkpoint-N}/`
  - Phase C: `_hungarian_eval.py score` → `hungarian_metrics.json`, 이어서 `select --eval-dir .../${MODE}_world_model` → adapter 경로의 `BEST_CHECKPOINT` (plain + json) 기록. baseline 은 `eval_dir` 의 형제 `base/` 에서 자동 로드해 비교 표에 포함된다.
...
- [`scripts/stage2_eval.sh`](./scripts/stage2_eval.sh)
  - 표준 실행 순서: **train → eval → merge**. eval 은 로컬 per-epoch checkpoint 만 소비하며 HF Hub 의존이 없다.
  - Phase A (baseline zero-shot, mode 무관) + Phase B (두 variant 각각 `outputs/{DS}/adapters/…/checkpoint-*` per-epoch sweep, backend 독립).
    - `lora_base`: `$BASE_MODEL` + `outputs/{DS}/adapters/{MODEL}_stage2_lora_base/checkpoint-*` (mode 무관)
    - `lora_world_model_from_${MODE}`: **로컬** `outputs/{DS}/merged/{MODEL}_stage1_${MODE}/` 를 base 로, `outputs/{DS}/adapters/{MODEL}_stage2_lora_world_model_from_${MODE}/checkpoint-*` adapter 를 얹어 로드. merged 디렉토리 없으면 hard-fail (stage1_merge 선행 필수).
    - `vllm_config` 에 dataset-specific `max_lora_rank` (MB=16, AC=32) 주입.
  - `LlamaFactory/scripts/vllm_infer.py` 호출 시 `cd "$LF_ROOT"` 내부에서 실행하고 `--dataset_dir '$LF_ROOT/data'` 절대 경로 필수.
  - Phase C: `_action_eval.py score` → `action_metrics.json`, 이어서 `select --eval-dir .../{variant}` → 각 adapter 경로의 `BEST_CHECKPOINT` (plain + json) 기록.
