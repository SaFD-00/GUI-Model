...
1. Section 0: 환경 설정, 모델/데이터셋 config 정의, Stage 1 학습 YAML (full · lora 양쪽) / Stage 2 학습 YAML (base · world-model-full · world-model-lora) / Stage {1,2} merge YAML 생성. **Stage 1 eval YAML 은 더 이상 생성하지 않는다** (eval 이 YAML 없이 HF Hub merged 모델을 직접 sweep).
...
4. Section 4: **Stage 1 merge (모든 epoch 각각 merge + HF Hub push. `-epoch{E}` 접미사)**
5. Section 5: **Stage 1 평가 (HF Hub epoch 별 merged 모델 sweep) 및 Hungarian F1 winner 선택**
6. Section 6: Stage 2 LoRA fine-tuning (world-model variant 는 Stage 1 `BEST_CHECKPOINT.json.epoch` 을 읽어 local `merged/.../epoch-{Ewin}/` 를 base 로 사용)
7. Section 7: **Stage 2 merge (variant × 전 epoch 각각 merge + HF Hub push)**
8. Section 8: **Stage 2 평가 (HF Hub sweep) 및 Step Accuracy winner 선택**
...
- [`scripts/stage1_eval.sh`](./scripts/stage1_eval.sh)
  ...
  - Phase C: `_hungarian_eval.py score` → `hungarian_metrics.json`, 이어서 `select --eval-dir .../${MODE}_world_model --hf-repo-template "SaFD-00/...-epoch{epoch}"` → adapter 경로의 `BEST_CHECKPOINT` (plain `epoch-{E}`) + `BEST_CHECKPOINT.json` (`epoch`, `hf_repo_id` 필드 포함) 기록. baseline 은 형제 `base/` 에서 자동 로드해 비교 표에 포함된다.
  - **재실행 시 skip (idempotent resume)**: baseline / epoch sweep unit 은 marker `hungarian_metrics.json` 존재 시, winner select 는 `BEST_CHECKPOINT.json` 존재 시 건너뛴다 (`_common.sh::skip_if_done`). 단 Phase B 에서 신규 평가가 하나라도 생기면 winner 는 강제로 재선정 (새 epoch 결과가 추가됐을 수 있음). 강제 재평가는 해당 marker 를 `rm` 한 뒤 재실행.
  - 분할 스크립트: [`scripts/stage1_eval_base.sh`](./scripts/stage1_eval_base.sh) (Phase A 단독), [`scripts/stage1_eval_world_model.sh`](./scripts/stage1_eval_world_model.sh) (Phase B + C 단독). 동일한 skip 규칙을 공유한다.
...
- [`scripts/stage2_train.sh`](./scripts/stage2_train.sh)
  ...
  - world-model variant 의 S1 base 경로는 런타임에 재작성된다: `adapters/{M}_stage1_${MODE}/BEST_CHECKPOINT.json.epoch` 을 읽어 local `merged/{M}_stage1_${MODE}/epoch-{Ewin}/` 로 YAML `model_name_or_path` 를 sed 로 치환 (임시 YAML 사용). `BEST_CHECKPOINT.json` 미존재 시 hard-fail — `stage1_eval.sh` 선행 필수.
...
- [`scripts/stage2_merge.sh`](./scripts/stage2_merge.sh)
  - 표준 실행 순서: **train → merge → eval**. 각 variant 의 `adapters/.../checkpoint-*` 전수 loop.
  - world-model variant 는 S1 winner merged (`merged/{M}_stage1_${MODE}/epoch-{Ewin}/`) 를 base 로, `lora_base` variant 는 `$BASE_MODEL` 을 base 로 사용.
  - `backend=llamafactory`: 임시 merge YAML 에 `model_name_or_path: {base}` + `adapter_name_or_path: {ckpt}` + `finetuning_type: lora` 삽입 → `llamafactory-cli export`
  - `backend=unsloth`: `scripts/_unsloth_merge.py --mode lora` → `save_pretrained_merged(method="merged_16bit")`
  - 산출물 (epoch 별): `outputs/{DS}/merged/{MODEL}_stage2_lora_{base|world_model_from_${MODE}}/epoch-{E}/` + HF Hub push `SaFD-00/...stage2-{base|${MODE}-world-model}-epoch{E}`. HF repo id 는 `_common.sh::hf_repo_id_stage2` 단일 정의.
- [`scripts/stage2_eval.sh`](./scripts/stage2_eval.sh)
  - Phase A (baseline zero-shot, mode 무관) + Phase B (`--epochs` 플래그로 받은 정수 리스트를 따라 두 variant 각각 **HF Hub merged repo sweep**, 기본 `1,2,3`, backend 독립).
    - `lora_base`: `SaFD-00/...stage2-base-epoch{E}` (mode 무관)
    - `lora_world_model_from_${MODE}`: `SaFD-00/...stage2-${MODE}-world-model-epoch{E}`
    - merged 이므로 `--adapter_name_or_path` · `max_lora_rank` 전부 제거. 로컬 `adapters/.../checkpoint-*` 나 `merged/` 디렉토리는 조회하지 않는다.
  - 결과 경로: `outputs/{DS}/eval/{MODEL}/stage2_eval/{base, {variant}/epoch-{E}}/`
  - `LlamaFactory/scripts/vllm_infer.py` 호출 시 `cd "$LF_ROOT"` 내부에서 실행하고 `--dataset_dir '$LF_ROOT/data'` 절대 경로 필수.
  - Phase C: `_action_eval.py score` → `action_metrics.json`, 이어서 `select --eval-dir .../{variant} --hf-repo-template "SaFD-00/...-epoch{epoch}"` → 각 adapter 경로의 `BEST_CHECKPOINT` + `BEST_CHECKPOINT.json` (`epoch`, `hf_repo_id` 필드 포함) 기록.
  - **재실행 시 skip (idempotent resume)**: baseline / epoch sweep unit 은 marker `action_metrics.json` 존재 시, winner select 는 variant 별 `BEST_CHECKPOINT.json` 존재 시 건너뛴다 (`_common.sh::skip_if_done`). Phase B 에서 신규 평가가 하나라도 생기면 해당 variant winner 는 강제로 재선정. `SWEEP_RAN` flag 는 variant 루프 진입 시마다 0 으로 초기화되어 두 variant 가 독립적으로 판정된다.
  - 분할 스크립트: [`scripts/stage2_eval_base.sh`](./scripts/stage2_eval_base.sh) (Phase A 단독), [`scripts/stage2_eval_world_model.sh`](./scripts/stage2_eval_world_model.sh) (Phase B + C 단독, 레거시 `lora_world_model` variant 고정 — `{MODE}-world-model` prefix 미적용). 동일한 skip 규칙을 공유한다.
...
bash scripts/stage1_eval.sh  --model qwen3-vl-8b --dataset MB --epochs 1,2,3         # HF sweep + winner
bash scripts/stage1_train.sh --model gemma-4-e2b --dataset MB --stage1-mode lora     # LoRA 모드
bash scripts/stage2_eval.sh  --model qwen3-vl-8b --dataset MB --epochs 3             # 최종 epoch 만 spot check
bash scripts/stage2_train.sh --stage1-mode lora                                      # 전체 모델, world-model 은 S1-lora winner 기준
bash scripts/stage1_train.sh                                                         # 전체 모델 + 전체 DS + full
```

> `--epochs` 는 `stage{1,2}_eval.sh` 전용 플래그 (기본 `1,2,3`). `_common.sh::parse_args` 에서 공통 파싱하지만 train/merge 스크립트는 이 값을 참조하지 않으므로 backward compat 유지.
...
raw JSONL + screenshots
  ...
  -> [per model] Stage 1 train  (mode = full | lora)
       → adapters/{M}_stage1_{mode}/checkpoint-*/
  -> [per model] Stage 1 merge (모든 epoch 각각)
       → merged/{M}_stage1_{mode}/epoch-{E}/   +   HF Hub ...-stage1-{mode}-world-model-epoch{E}
  -> [per model] Stage 1 eval (HF Hub sweep)
       → eval/{M}/stage1_eval/{MODE}_world_model/epoch-{E}/
       → BEST_CHECKPOINT.json.{epoch, hf_repo_id}   (in adapters/..._stage1_{mode})
  -> [per model] Stage 2 train (base + world-model-{mode})
       world-model base = merged/{M}_stage1_{mode}/epoch-{Ewin}/   (local, Ewin from BEST_CHECKPOINT.json)
       → adapters/{M}_stage2_lora_{base|world_model_from_{mode}}/checkpoint-*/
  -> [per model] Stage 2 merge (variant × 전 epoch)
       → merged/{M}_stage2_lora_{variant}/epoch-{E}/  +  HF Hub ...-stage2-{base|{mode}-world-model}-epoch{E}
  -> [per model] Stage 2 eval (HF Hub sweep)
       → eval/{M}/stage2_eval/{variant}/epoch-{E}/
       → BEST_CHECKPOINT.json  (in adapters/..._stage2_lora_{variant})
```
...
│   │   ├── checkpoint-*/
│   │   ├── BEST_CHECKPOINT               # 예: "epoch-3"
│   │   └── BEST_CHECKPOINT.json          # epoch, hf_repo_id, candidates 포함
...
│       ├── base/
│       ├── lora_base/epoch-{E}/
│       ├── lora_world_model_from_full/epoch-{E}/
│       └── lora_world_model_from_lora/epoch-{E}/
└── merged/
    ├── {model_short_name}_stage1_full/epoch-{E}/
    ├── {model_short_name}_stage1_lora/epoch-{E}/
    ├── {model_short_name}_stage2_lora_base/epoch-{E}/
...
| Stage / variant | 패턴 |
|-------|------|
| Stage 1 (full FT) | `SaFD-00/{short_name}-{slug}stage1-full-world-model-epoch{E}` |
| Stage 1 (LoRA)    | `SaFD-00/{short_name}-{slug}stage1-lora-world-model-epoch{E}` |
| Stage 2 base      | `SaFD-00/{short_name}-{slug}stage2-base-epoch{E}` |
| Stage 2 world (from S1 full) | `SaFD-00/{short_name}-{slug}stage2-full-world-model-epoch{E}` |
| Stage 2 world (from S1 lora) | `SaFD-00/{short_name}-{slug}stage2-lora-world-model-epoch{E}` |

`{slug}` 는 `mb-` (MobiBench) 또는 `ac-` (AndroidControl). `{E}` 는 각 `checkpoint-*/trainer_state.json` 의 `epoch` 을 `int(round(...))` 로 추출한 값. 조립은 `scripts/_common.sh::hf_repo_id_stage{1,2}` 에 중앙화.
...
- winner 기록 위치 (MODE = `full` | `lora`):
  ...
  - `outputs/{DS}/adapters/{MODEL}_stage1_${MODE}/BEST_CHECKPOINT.json` (`epoch`, `hf_repo_id`, `metric_value`, `candidates`[])
...
- winner 기록 위치 (plain + JSON. JSON 에 `epoch`, `hf_repo_id` 포함):
  - `outputs/{DS}/adapters/{MODEL}_stage2_lora_base/BEST_CHECKPOINT[.json]`
  - `outputs/{DS}/adapters/{MODEL}_stage2_lora_world_model_from_${MODE}/BEST_CHECKPOINT[.json]`
...
- merge 스크립트는 `outputs/{DS}/adapters/.../checkpoint-*` 가 하나라도 없으면 hard-fail. `BEST_CHECKPOINT` 에 더 이상 의존하지 않는다 (전 epoch loop).
- Stage 2 train/merge 는 Stage 1 winner (`BEST_CHECKPOINT.json.epoch`) 와 로컬 `outputs/{DS}/merged/{MODEL}_stage1_{full|lora}/epoch-{Ewin}/` 에 의존한다. 즉 `stage1_eval.sh` 가 선행해야 `stage2_train.sh` / `stage2_merge.sh` 가 동작.
- Stage 2 eval 은 HF Hub push 된 stage2 merged repo 만 참조하므로 로컬 S1 merged 에 의존하지 않는다.
