...
├── Section 1-2: 데이터 등록 (상대 경로로 dataset_info.json 등록, JSONL images 는 `images/...` 로 유지)
...
| 데이터셋 | 이미지 수 | 경로 패턴 |
|----------|----------|----------|
| MobiBench | 3,655 개 | `images/episode_{id:06d}_step_{idx:04d}.png` |
| AndroidControl | 20,129 개 (추출 필요: `scripts/extract_androidcontrol_images.py`) | `images/episode_{id:06d}_step_{idx:04d}.png` |

- Stage 1/2 공유
- `image_max_pixels`: 4,233,600
- 각 JSONL 파일의 `images` 는 해당 JSONL 위치 기준 상대 경로로 해석 (파일 복사 없음)
...
JSONL 파일을 LlamaFactory/data/ 로 복사하지 않는다. 각 JSONL 내부의 `images` 값은 해당 JSONL 파일 기준 상대 경로(`images/...`)를 사용한다.
