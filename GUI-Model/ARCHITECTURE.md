...
2. Section 1-2: 데이터 준비. 노트북은 `LlamaFactory/data/dataset_info.json` 을 갱신(AC/MC 는 `*_stage1_train/test`, AC 는 추가로 `*_stage2_train/test_id/test_ood`, MobiBench 는 `_EVAL_ONLY_BENCHMARKS` 루프가 단일 파일 entry 등록). MB 단일 엔트리는 `scripts/_common.sh::ensure_eval_only_dataset_info()` 가 eval script source 시점에 idempotent 하게도 보장하므로, notebook 없이 `stage{1,2}_eval.sh --eval-datasets MB` 를 바로 돌릴 수 있다.
...
- **Stage 2 (AC only)**: 같은 (id_apps, ood_apps) 에서 각 풀별 action-type stratified 샘플링 (largest-remainder). train 은 `null` primary_app 에피소드까지 흡수해 regular 크기 유지.
...
- [`scripts/split_data.py`](./scripts/split_data.py) 가 Stage 1 + Stage 2 ID/OOD split 을 담당한다. 기본 크기: 양 stage 모두 train 50K / test_id 3K / test_ood 3K (`--stage1-*-size`, `--stage2-*-size`).
...
- [`scripts/eval_viewer.py`](./scripts/eval_viewer.py): Stage 1/2 산출물 비교 HTML 빌더. 각 `eval/{MODEL}/stage{1,2}_eval/` 에 `pairs_{on-AC,on-MB}.html` + `pairs_summary.md` 생성, in-page 체크박스로 variant/epoch 컬럼 토글
