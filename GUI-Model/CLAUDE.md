...
`gui-model.ipynb` Cell 3에서 `DATASET_NAME` 변수로 학습 데이터셋을 전환한다:

```python
DATASET_NAME = "MobiBench"       # 소규모 (~6K, 기본값)
DATASET_NAME = "AndroidControl"  # 대규모 (~93K)
```
...
| Output prefix | (없음) | `AC/` |
| HF slug | (없음) | `ac-` |
| Stage 1 epochs / LR | 3.0 / 2e-5 | 1.0 / 1e-5 |
| Stage 2 epochs / LR | 2.0 / 5e-5 | 1.0 / 2e-5 |

모든 파생 변수(데이터셋 등록명, HF 모델 ID, 출력 경로, 하이퍼파라미터)는 `DATASET_NAME` 하나로 자동 결정된다. `MobiBench` 설정 시 기존과 완전히 동일하게 동작한다.
...
[gui-model.ipynb]  ──  전체 파이프라인 오케스트레이션 (Jupyter 노트북)
       ...
       └── Section 8: Stage 2 평가 (4-Way + Baseline 비교)
```
...
- **데이터셋 선택**: `gui-model.ipynb` Cell 3의 `DATASET_NAME` 변수 (`"MobiBench"` 또는 `"AndroidControl"`)
- **데이터 등록**: Cell 8/11 실행 시 `dataset_info.json`에 상대 경로(`../../data/{DATASET_NAME}/...`)로 자동 등록. 파일 복사 없음
- **Stage 1 학습**: `LlamaFactory/examples/train_custom/GUI-Model/stage1_full/qwen3_vl_8b_gui.yaml` (노트북에서 동적 생성)
...
|------|-------------------|----------------|
| batch × accum × GPU | 2 × 8 × 4 = 64 | 1 × 16 × 2 = 32 |
...
- **학습된 모델 (HuggingFace)**:
  - MobiBench: `SaFD-00/qwen3-vl-8b-stage1-world-model`, `stage2-world-model`, `stage2-base`, `stage2-gworld`
  - AndroidControl: `SaFD-00/qwen3-vl-8b-ac-stage1-world-model`, `ac-stage2-world-model`, `ac-stage2-base`, `ac-stage2-gworld`
