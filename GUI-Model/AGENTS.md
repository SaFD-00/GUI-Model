...
- **Stage 1 평가**: [`scripts/_hungarian_eval.py`](./scripts/_hungarian_eval.py) 가 기준. 흐름은 **train → merge → eval** — `stage1_merge.sh` 가 모든 epoch 를 각각 local merge + HF push 한 뒤, `stage1_eval.sh` 가 baseline + 각 epoch 의 HF Hub merged repo (`SaFD-00/...stage1-${MODE}-world-model-epoch{E}`) sweep 을 돌고 `avg_hungarian_f1` winner 를 adapter 경로의 `BEST_CHECKPOINT`(plain `epoch-{E}`) + `BEST_CHECKPOINT.json`(`epoch`, `hf_repo_id` 필드 포함) 에 기록한다. epoch 은 각 `checkpoint-*/trainer_state.json` 에서 `int(round(...))` 로 추출 (`_common.sh::ckpt_epoch_from_dir`). 정본은 notebook 의 Stage 1 eval 셀 (Section 5).
- **Stage 2 평가**: [`scripts/_action_eval.py`](./scripts/_action_eval.py) 가 기준 (winner metric `step_accuracy`). 흐름은 `stage2_train.sh` → `stage2_merge.sh` (variant × 전 epoch push) → `stage2_eval.sh` (variant × 각 epoch HF Hub sweep). 로컬 `merged/{MODEL}_stage1_${MODE}/` 에 의존하지 않고 HF Hub `SaFD-00/...stage2-{base|${MODE}-world-model}-epoch{E}` 만 pull. Stage 2 world-model **train/merge** 는 여전히 로컬 S1 winner merged (`epoch-{Ewin}/`) 를 base 로 사용 (`BEST_CHECKPOINT.json.epoch` 자동 해석). 정본은 notebook Section 8, 회귀 테스트 [`tests/test_action_eval.py`](./tests/test_action_eval.py). 메트릭 정의는 [`ARCHITECTURE.md`](./ARCHITECTURE.md) §6 참고.
...
- shell script CLI 는 `--model MODEL --dataset DS --stage1-mode {full|lora}` 플래그 방식이다 (기본: full).
