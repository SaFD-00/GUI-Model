...
Two jobs behind ``atlas-collect sync-installed`` and ``atlas-collect provision``.
...
1. :data:`APK_CACHE_DIRNAME` — Monkey-Collector's 129-APK local cache. First
   because it is offline, already ABI-checked, and F-Droid drops apps from its
   index without notice, which makes the local cache the only durable copy.
...
from atlas_collector.catalog import AppRow
from atlas_collector.paths import catalog_path, project_root
...
#: Monkey-Collector sits beside this repo; AndroidWorld's pinned APKs live in
#: EpochDroid, one level further up. Both are derived from ``project_root()``
#: rather than hard-coded absolute paths so a clone elsewhere still resolves.
APK_CACHE_DIRNAME = Path("Monkey-Collector") / "catalog" / "apks"
...
def apk_cache_dir() -> Path:
    """Monkey-Collector's offline APK cache (sibling checkout)."""
    return project_root().parent / APK_CACHE_DIRNAME
...
def download_dir() -> Path:
    ...
    Downloads are NOT written into Monkey-Collector's cache: that directory is
    another project's committed asset, and a half-written file there would look
    like a cache hit to every later run.
    """
