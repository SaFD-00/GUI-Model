...
only against ``tmp_path``: ``csv.writer`` defaults to QUOTE_MINIMAL and several
committed rows are quoted gratuitously (apostrophes but no comma), so a
read/write round-trip is NOT byte-identical and must never be asserted as such.
...
from atlas_collector.catalog import (
    FIELDNAMES,
    PENDING_PACKAGE,
    VALID_STATUSES,
    AppRow,
