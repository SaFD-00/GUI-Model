...
15d. **AC_EXP08 = 3-포맷 관측성 분할(anti-copy) stage1 실험군**이다 (좌표계·budget·cutoff 는 EXP05/07 과 같고 자격은 `qwen2.5-vl-3b`·`qwen2.5-vl-7b`). 데이터는 EXP01/05/07 파생이 **아니고** `data/AndroidControl/EXP08_*.jsonl` (0822 Cerebra 파서 재추출) 에서 자체 빌드한다 — 다만 이미지는 공용 `AndroidControl/images/` 를 그대로 참조한다 (zero-pad remap, 전수 매핑 확인). **stage1 의 누출 차단은 에피소드 단위 홀드아웃**이다. state test 는 `full`/`masked`/`dropped` 3 파일이며 **같은 원본에서 나온다** (sample_id 집합·`raw_current_state` 동일이 불변식). 빌드 정본은 train 이 `scripts/build_exp08_data.py`(stage1) · `scripts/build_exp08_stage2_v2.py`(stage2) 이고, **eval 은 `scripts/build_exp08_eval_v2.py`** 다 (2026-09-08 전면 교체 — 아래 15j). ⚠️ **"원본에 앱 파티션 메타가 없다" 는 틀렸다** (2026-08-28 정정 — 이 문장을 근거로 앱 분할을 포기하지 마라): `data/AndroidControl/episodes_meta.jsonl` 의 `primary_app` 이 EXP08 에피소드를 **전수 커버**한다. 그 사실 위에서 구 stage2 는 앱 축 4 + step 축 3 버킷을 가졌다 — **그 7 버킷은 2026-09-08 eval v2 로 은퇴**했고 (아카이브 `stage2_test_*_2026-08-28.jsonl`), 지금의 축은 trajectory ID/OOD 다 (15j). **stage1 산출물 5 파일(`stage1_train.jsonl` + `stage1_test_*.jsonl` 4 종)은 학습·평가가 끝난 불가침 자산이라 바이트 불변이어야 한다** — stage2 빌더가 sha256 을 빌드 전후로 대조해 강제한다. [§3 계보](./ARCHITECTURE.md#3-데이터와-설정-계약)
...
15f. **Cerebra 스키마를 채점하려면 `--xml-schema cerebra` 를 줘라 — `_action_eval.py` 도 포함이다.** `scripts/_hungarian_eval.py` / `_state_diff_eval.py` / **`_action_eval.py`** 의 기본값은 전부 `android` 이고, 기본값으로 EXP08 을 채점하면 **에러 없이** 지표가 무너진다. 반대로 기존 실험군에 `cerebra` 를 주지 마라 — 채점 기준이 바뀐다 (opt-in 규약: 기존 실험군의 채점 결과는 불변이어야 한다). ⚠️ **"셸을 통하면 안전하다" 는 전제는 틀렸었다** (2026-08-28 실측): `stage1_eval.sh` 의 action 분기가 `schema_flag=""` 로 하드코딩돼 있어 플래그가 채점기에 **가지 않았고**, EXP08 action 13 leaf 전부가 click/long_press 를 `no_bbox` 로 처리해 `cond_bbox_acc = 0.0000` 이 났다. 위험한 자리는 셋이다 — (a) 채점기를 **직접** 실행할 때, (b) 새 Cerebra 계열 DS 를 `ds_xml_schema_flag` 에 등록하지 않았을 때, (c) **채점 분기가 플래그를 안 받도록 배선돼 있을 때**. (c) 가 가장 조용하다: 셸도 채점기도 정상으로 보인다. 채점기에 인자가 생기면 **그 인자를 넘기는 셸 분기를 전수로 확인하라.** [§6 메트릭](./ARCHITECTURE.md#6-메트릭)
15g. **xy 계열 판정(`pos`/`xy` 채점 모드)의 정본은 `lf_registry.PIXEL_XY_DATASETS` 하나다 — 네 번째 장소에 복제하지 마라.** 소비자는 둘뿐이다: `scripts/_compare_site.py::XY_FAMILY` 는 `pixel_xy_ds_keys()` 로 **유도**하고, `scripts/_common.sh::ds_is_pixel_xy` 는 성능상 case 목록을 셸에 남기되 — **셸 안의 유일한 목록이다**. 다른 셸 스크립트는 목록을 다시 적지 말고 `ds_is_pixel_xy` / `ds_score_mode_flag` 를 호출해라 (`stage2_eval.sh` 가 인라인 복제본을 갖고 있다가 2026-08-28 에 위임으로 정리됐다) — `tests/test_pixel_xy_consistency.py` 가 셸 원문을 파싱해 드리프트를 실패로 잡는다 (셸을 런타임에 파이썬으로 유도하지 않는 것은 의도적 트레이드오프 — eval 경로에 인터프리터 기동을 끼우지 않는다). 이 규칙이 필요한 이유는 **같은 사고가 두 번 났기 때문**이다: 1차는 `rebuild_*.sh` 3 종이 각자 case 문을 복제하다 하나만 EXP06 을 갖고 있던 것(2026-08-03), 2차는 그 셸 통합이 **언어 경계를 넘지 않아** `XY_FAMILY` 가 AC_EXP08 을 놓친 채 남아 사이트가 조용히 index 모드로 채점하던 것(2026-08-28)이다. 어긋나면 **에러 없이** 전 건이 오답이 된다. [§6 메트릭](./ARCHITECTURE.md#6-메트릭)
15h. **AC_EXP08 stage1 ablation 을 만질 때 두 가지를 지켜라 — 둘 다 어기면 조용히 깨진다.** (a) **`"fmt" 키 부재 ⇒ action` 규약을 새 파일에 재사용하지 마라.** 그 규약은 `build_exp08_ablation_data.py` 가 부모 `stage1_train.jsonl`(state 는 전부 `fmt` 보유, downstream 은 미보유)에서 action 만 뽑는 데 쓰는 것이고, **inverse 레코드가 같은 파일에 들어오는 순간 inverse 가 action 으로 읽힌다**. `stage1_train_inverse_mix.jsonl` 은 모든 행에 명시적 `task`(`forward`/`inverse`/`action`) 를 싣는다 — 새 혼합 데이터를 만들면 같은 방식을 따르고, `fmt` 로 task 를 추론하지 마라. (b) **inverse 표본을 In-domain 앱(`APP_BOTH ∪ APP_S1_ONLY`) 밖에서 뽑지 마라.** `APP_OOD`/`APP_S2_ONLY` 앱이 stage1 train 에 한 건이라도 들어가면 "OOD 앱은 stage1 train 에 한 번도 등장하지 않는다" 가 무효가 되고 **stage2 앱 축 eval 4 버킷 전체가 의미를 잃는다** — 앱 목록은 `stage2_train.jsonl.meta.json::app_lists` 에서 읽고 재계산하지 마라. 누출 제외는 **에피소드 통째**여야 한다 (step 단위로 빼면 같은 에피소드의 나머지가 남는다). 빌드 정본은 `scripts/build_exp08_inverse_mix_data.py` 이고 `--verify-only` 가 (a)(b) 를 fail-closed 로 검사한다. [§3 계보](./ARCHITECTURE.md#3-데이터와-설정-계약)

15i. **stage1 ablation variant 세그먼트를 두 번째 장소에 적지 마라 — 정본은 `_common.sh::stage1_variant_seg` 하나다.** variant 목록도 하드코딩하지 말고 `lf_registry::stage1_extra_variants` 에서 유도해라 (`stage1_known_variants`). 이 규칙이 필요한 이유: `stage1_merge.sh` 가 variant 를 모른 채 variant 없는 경로를 만들던 시기에 ablation 을 merge 하면 **조용히 메인 런의 어댑터를 merge 해 같은 HF repo id 로 push** 했다 — HF 의 stage1 체크포인트를 복구 불가로 덮어쓴다. 지금은 `assert_variant_segment` 가 adapters 입력·merged 출력·HF repo id **세 지점**에서 세그먼트 부재를 실패로 잡는다. **variant 가 없을 때의 문자열은 바이트 불변이어야 한다** (메인 런 산출물·HF repo·기존 eval leaf 가 움직이면 안 된다) — `tests/test_stage1_variant_wiring.py` 가 양쪽 방향을 고정한다. [§5](./ARCHITECTURE.md#5-실행-데이터-흐름과-산출물)

15j. **AC_EXP08 eval 은 2026-09-08 에 task 축으로 전면 교체됐다 — 네 가지를 지켜라.** (a) **train 4 파일(메인 stage1·stage2 + ablation 2 종)을 다시 굽지 마라.** 새 eval 의 ID/OOD 는 그 train 의 에피소드 집합을 기준으로 갈렸다 — 재빌드하는 순간 11 파일의 라벨이 **에러 없이** 전부 거짓이 된다. `build_exp08_stage2_v2.py` 는 `eval_v2.meta.json` 이 있으면 abort 하고, 뚫으려면 `--force-rebuild` 로 의도를 명시해야 한다 (그리고 eval 도 함께 재빌드해야 한다). (b) **OOD 라벨을 계보를 가로질러 읽지 마라.** `action_test_s1_ood` 는 `stage1_train` 기준, `action_test_s2_ood` 는 `stage2_train` 기준이고, **두 계보 공통의 자는 `action_test_ood`(엄격) 하나뿐**이다. 셸은 "양쪽 전량" 결정에 따라 5 파일을 다 돌리므로 **걸러 읽는 것은 읽는 쪽 책임**이다. `state_test_ood` 도 85 행이 (F,T) 셀 보충분이라 stage2 계보로 읽을 땐 `s2_ep_seen=false` 로 필터해야 한다. (c) **"앱 축 OOD" 를 다시 만들려 하지 마라** — train 동결 상태에서 train 미등장 앱은 **1 개(4 step)** 다 (실측). 구 stage2 의 앱 축 4 버킷은 "action 지도를 안 받은 앱" 이라는 완화된 정의였고 v2 에서 은퇴했다. (d) **11 파일의 action mix 가 같다는 것이 이 설계의 핵심**이다 — 한 파일만 다시 뽑아 mix 가 어긋나면 버킷 간 원값 비교가 무효가 되고 §6 (c) 의 macro 보정 시절로 되돌아간다. 쿼터는 셀 단위 2-pass 이고 **한 셀이 k 개 버킷을 공급하면 가용량을 k 로 나눠야** 한다. 배선 드리프트는 `tests/test_exp08_eval_v2_wiring.py` 가 잡는다. [§3 eval v2](./ARCHITECTURE.md#3-데이터와-설정-계약) · [§6 (c)](./ARCHITECTURE.md#6-메트릭)

15k. **AC_EXP08 stage2 데이터 ablation(`action-distribution`)을 만질 때 두 가지를 지켜라.** (a) **채움 표본을 현 `stage2_train` 의 에피소드 집합 밖에서 뽑지 마라** — 밖에서 뽑는 순간 `action_test_s2_ood`·`action_test_ood` 의 "이 trajectory 는 stage2 train 에 없다" 가 **에러 없이** 거짓이 된다 (15j(b) 와 같은 실패 유형). eval 11 파일의 step 자체도 전량 제외한다. 빌더 `verify()` 가 "부모 밖 에피소드 = 0" 과 "∩ eval = 0" 을 fail-closed 로 잡는다. (b) **`stage1_train_inverse_mix.jsonl` 을 제외 대상에 넣지 마라** — 그것은 `stage2_train` **이후에** 만들어져 이미 2,538 step 을 공유한다. 넣으면 없는 제약을 만들어 희소 action 재고를 과소평가하고(실측: `open` 채움 여유가 99 → 30 으로 잘못 줄었다) 도달 가능한 N 을 실제보다 낮게 판단하게 된다. stage2 데이터의 제외 기준은 **메인 stage1** (`stage1_train` + `action_only`) 이다. 빌드 정본 `scripts/build_exp08_action_dist_data.py`. [§3](./ARCHITECTURE.md#3-데이터와-설정-계약)

16. **`SMOKE=1` override 의 따옴표를 살려라.** OmegaConf 가 따옴표 없는 값을 boolean 으로 파싱해 HF 가 죽는다. [§4 함정 17](./ARCHITECTURE.md#4-파이프라인-컴포넌트)
17. **데이터 파일명 규칙을 지켜라 — 디렉토리가 실험을 말하면 파일은 침묵한다.** `AndroidControl_EXP{NN}/`·`MonkeyCollection/`·`MobiBench/` 안은 `stage{N}_<role>.jsonl` 이고 — **분할축이 stage 가 아니라 task 면 task 를 앞에 쓴다** (`state_test_{id,ood}_{fmt}.jsonl` · `action_test_{bucket}.jsonl`, AC_EXP08 eval v2 가 유일한 사례). 세대 교체로 은퇴한 eval 은 **`_{YYYY-MM-DD}` 접미**로 남기고 등록 키도 같이 접미를 단다 (재채점 경로를 살려 둔다) —, 공유 폴더 `data/AndroidControl/` 의 변형본만 `EXP{NN}_` 접두를 쓴다 (표현 서픽스 `_xy`/`_pixel-aligned`/`_filtered` 는 **쓰지 않는다** — EXP 소속이 표현을 함의한다). 멀티-exp 공유 파일은 **직접 소비하는 최초 EXP 하나**만 소유하고 재사용 관계는 문서가 기록한다. 원본 3 종 (`implicit-world-modeling_*`) 과 `episodes_meta.jsonl`·`images/` 는 이름을 바꾸지 않는다. **빌더 입력 원천도 예외가 아니다 — `*_src` 전용 디렉토리를 만들지 마라** (벤더 파일명을 버리고 공유 폴더에 `EXP{NN}_` 접두로 평탄하게 놓는다; 한 파일이 여러 stage 를 먹이면 **최대 소비자** 이름, 균등 분배라 최대 소비자가 없으면 **stage 중립명**). [§3 데이터 파일명 규칙](./ARCHITECTURE.md#3-데이터와-설정-계약)
