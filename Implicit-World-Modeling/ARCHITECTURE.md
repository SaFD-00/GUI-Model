...
> **표 밖의 예외 — `_FORCE_HALF_BATCH_DATASETS` (EXP07 전용).** 위 표의 "반감 면제" 는 optimizer state 메모리 근거인데, half-batch 는 activation(LM head logits = seq_len × vocab) 문제라 **축이 다르다**. EXP07 은 1080×2400 + cutoff 24576 조합에서 `pdbs=2` 일 때 step 28 에 **logits 23.77 GiB 단일 할당으로 확정 OOM** 했다 (2026-07-28 stage2 base 실측) → 면제를 다시 취소해 `pdbs=1 / ga=32` 로 강제한다.
>
> **EXP08 은 2026-09-01 사용자 결정으로 이 예외에서 빠졌다** → 80GB 에서 면제를 받아 `pdbs=2 / ga=16` 으로 돈다 (global batch 64 불변). ⚠️ **이 결정은 실측 OOM 을 이긴 결정이지 실측을 반증한 것이 아니다** — 2026-08-31 A100×2 stage2 lora 실측에서 `pdbs=2` 는 스파이크만 키운 게 아니라 baseline 을 30.7 → 63.2 GB 로 같이 끌어올려 남은 여유가 14.10 GiB 였고, 거기에 24.49 GiB 단일 할당이 들어와 **world-model·base 두 계보가 똑같이 step 31 에서 OOM** 했다 (EXP07 은 step 28). 그 실측은 지금도 사실이고, **OOM 이 재발하면 되돌릴 자리는 `_FORCE_HALF_BATCH_DATASETS` 의 한 줄**이다 — 그 뒤에 다시 빼려면 근거는 "여유가 있어 보인다" 가 아니라 **`pdbs=2` 에서 실제로 완주한 로그**여야 한다. 헤드룸을 **바꾼 뒤의 설정**으로 재야 한다는 교훈 자체는 유효하다 (step 20 근처의 평평한 메모리는 근거가 못 된다 — 수렴이 아니라 그 구간 샘플이 짧았을 뿐이고, 데이터 max 는 p99 의 2.5 배다). 2026-09-01 신설되는 inverse 혼합 stage1 데이터는 프롬프트에 XML 이 2 개(current+next) 들어가 시퀀스가 더 길 수 있어 **OOM 재발 1순위 후보**다. 참고로 EXP08 은 `_HALF_BATCH_DATASETS` 에는 그대로 남아 있어 RTX5090 과 80GB × 7-9B × full 에서는 여전히 `pdbs=1 / ga=32` 다. 정본은 `scripts/gpu_policy.py::_FORCE_HALF_BATCH_DATASETS`, 방향 고정은 `tests/test_gpu_policy.py::test_exp07_force_half_batch_overrides_no_offload_exemption` (EXP07 강제) 와 `tests/test_gpu_policy.py::test_exp08_takes_half_batch_exemption_on_80gb` (EXP08 면제).
...
- **EXP08 빌드**: 빌드 정본 [`scripts/build_exp08_data.py`](./scripts/build_exp08_data.py) — **EXP08 train/test jsonl 의 유일한 커밋된 생성 경로**다. 원천은 `data/AndroidControl/` 의 0822 Cerebra 재추출본 2 파일이고, 이미지는 EXP07 과 같은 zero-pad remap 으로 공용 `AndroidControl/images/` 를 참조한다 (2026-08-22 전수 확인: 60,871 + 86,431 전부 실재 파일에 매핑). **이 데이터는 AndroidControl 을 Cerebra html-like 파서로 다시 뽑은 것**이라 계보상 AC 계열이 맞다 — 새 코퍼스가 아니다.
  ...
  - **stage2 데이터 ablation — `stage2_train_action_distribution.jsonl` (2026-09-08)**. 빌드 정본 [`scripts/build_exp08_action_dist_data.py`](./scripts/build_exp08_action_dist_data.py). `stage2_train`(30K) 의 **terminate 27.3%** 가 stage1 downstream(10K, 14.9%) 과 크게 달라, 그 분포를 맞춘 대조군이다. 방법은 **기존 샘플을 바이트 불변으로 두고 과대한 층만 드롭한 뒤 상한까지 채우는 것** — 유지 25,831 / 드롭 4,169(terminate 3,713 · type 456) / 추가 2,590 → **실현 28,421**.
    - ⚠️ **목표 분포에 정확히 도달할 수 없다.** 메인 stage1 이 희소 action 을 이미 소비했다 (`open` 5,386 중 3,532 · `navigate_back` 2,841 중 1,848). stage2 가 쓸 수 있는 전부가 1,854 / 993 이고 현 30K 가 1,267 / 660 을 쥐고 있어 채울 여유가 **+99 / +119** 뿐이다 → `open` 4.81%(목표 6.26%) · `navigate_back` 2.74%(3.29%). 규칙을 완화하지 않고 상한까지만 채운 뒤 실현치를 sidecar 에 남긴다. 정확 일치는 N≈21,800 에서만 가능한데 그러면 **분포와 규모가 함께 움직여 비교 변수가 둘**이 된다 (2026-09-08 사용자가 근사를 선택).
    - **eval 은 손대지 않고 라벨이 산다** — 채움 표본을 **현 `stage2_train` 의 에피소드 집합 안에서만** 뽑기 때문이다. 실측: `action_test_s2_ood`·`action_test_ood` 오염 **0행**, `action_test_s2_id` 는 드롭으로 에피소드가 빠져 **15행**이 "라벨은 ID 인데 이 variant 는 미관측" 이 된다 (sidecar `eval_label_drift`). eval 11 파일의 step 자체도 전량 제외한다.
    - **`inverse-mix` 를 제외 대상에 넣지 마라** — 그것은 `stage2_train` **이후에** 만들어져 이미 2,538 step 을 공유한다. 넣으면 없는 제약을 만들어 희소 action 재고를 과소평가한다 (1 차 계산이 실제로 그랬다).
    - **배선**: 레지스트리 `stage2_extra_variants` (stage1 축의 대칭이되 dataset 뿐 아니라 epochs·save_strategy·save_steps 까지 덮는다) → `gen_configs` 가 **stage2 full 만** `{base, world-model-full} × action-distribution` 4 YAML 을 렌더한다 (1 epoch · `save_steps 0.25` — 2026-09-08 사용자 지정). 산출 경로는 `-action-distribution` 접미로 메인 런과 갈린다.
...
> **`--stage1-variant` — stage1 ablation 계보 축 (2026-09-01 관통 배선).** `AndroidControl_EXP08` 만 갖는 축이고 값의 정본은 `lf_registry::_DATASET_CONFIG[ds]["stage1_extra_variants"]` 다 (`_common.sh::stage1_known_variants` 가 유도한다 — 셸에 목록을 적지 마라). 세그먼트는 **`-{variant}` 를 `world-model` 토큰 바로 뒤**에 넣고, 조립 정본은 `_common.sh::stage1_variant_seg` **한 곳**이다. 네 자리에 동시에 나타난다 — 학습 `output_dir`(커밋 YAML), merged 경로, `hf_repo_id_stage1`, 그리고 stage2 의 계보 키(`{M2}_world-model-{variant}_from_{M1}-ep{N}`)·eval leaf. **variant 를 안 주면 모든 문자열이 도입 전과 바이트 동일**하다 (메인 런 산출물·HF repo·기존 eval leaf 불변이 요구사항이다).
>
> ⚠️ **왜 가드가 필요한가.** 배선 전 `stage1_merge.sh` 는 이 축을 몰라 variant 없는 경로를 만들었고, ablation 을 merge 하면 **조용히 메인 런의 어댑터를 merge 해 같은 HF repo id 로 push** 했다 — HF 의 stage1 체크포인트를 복구 불가로 덮어쓴다. 그래서 `--stage1-variant` 자체를 `exit 2` 로 거절하는 임시 가드가 있었다. 지금은 `assert_variant_segment` 가 **merge·push 전에** 세 지점(adapters 입력 · merged 출력 · HF repo id)에서 세그먼트 부재를 `exit 1` 로 잡는다. 기능을 금지하는 대신 **실제 사고 조건**을 검사하므로 이후 리팩터링에도 유효하다. 양방향 고정은 `tests/test_stage1_variant_wiring.py`.
...
- **`stage2_train.sh`** — YAML `…/stage2_${MODE2}/{MODEL}_{base,world-model-full,world-model-lora}.yaml`. 실행 env 는 **Stage 1 과 동일**하다 — `FORCE_TORCHRUN=1 NNODES=1 NPROC_PER_NODE=…` (`stage2_train.sh:101` ↔ `stage1_train.sh:43`). 유일한 차이는 stage1 만 `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True` 를 추가로 export 한다는 것뿐이다. world-model variant 는 stage1 계보를 base 로 쓰도록 `model_name_or_path` 를 런타임 sed 치환하고 (그래서 YAML 의 HF id placeholder 는 무시된다), YAML 의 `__STAGE1_EPOCH__` 플레이스홀더가 `${N}` 으로 치환돼 결과가 `…_world-model_from_{M1}-ep{N}/` 로 분리 저장된다. **base 해석은 2026-09-01 부터 `resolve_eval_model_path` 위임이다 — local `merged/…/epoch-${N}/` 우선 + HF Hub fallback** (그전엔 디렉토리 미존재 시 hard-fail 이었다). 이유: EXP08 의 stage1 체크포인트 12 개는 HF 에만 있고 로컬엔 `epoch-3` 만 merged 라, "기존 런의 epoch-1 을 stage2 출발점으로 쓴다" 는 비교가 아예 불가능했다. 판정·repo id 조립을 다시 쓰지 않고 위임하므로 train 과 eval 이 갈릴 수 없다. ⚠️ **대가 — `--stage1-epoch` 오타가 학습 진입 전에 안 걸린다**: 존재하지 않는 HF repo id 를 반환하고 실패가 `llamafactory-cli train` 안의 **404** 로, GPU·DeepSpeed 초기화를 다 하고 나서 나타난다 (eval 경로가 예전부터 지던 트레이드오프다). `DRY_RUN=1` 이 해석된 id 를 먼저 보여준다.
