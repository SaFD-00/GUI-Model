...
> **표 밖의 예외 — `_FORCE_HALF_BATCH_DATASETS` (EXP07 · EXP08).** 위 표의 "반감 면제" 는 optimizer state 메모리 근거인데, half-batch 는 activation(LM head logits = seq_len × vocab) 문제라 **축이 다르다**. 1080×2400 + cutoff 24576 조합은 `pdbs=2` 에서 logits 단일 할당이 20 GiB 대로 뛴다 — EXP07 은 step 28 에 23.77 GiB 로(2026-07-28 stage2 base), **EXP08 은 step 31 에 24.49 GiB 로**(2026-08-31 stage2 lora, world-model·base 두 계보가 동시에) OOM 했다. 그래서 면제를 다시 취소해 `pdbs=1 / ga=32` 로 강제한다.
>
> ⚠️ **"GPU 에 여유가 있으니 EXP08 은 빼도 된다" 는 논증에 넘어가지 마라 — 2026-08-31 에 실제로 빼봤고 되돌렸다.** 그 논증은 `pdbs=1` 로 도는 중의 여유(30.7 GB 사용 / 49 GB 여유)를 `pdbs=2` 의 스파이크와 비교한다. **틀렸다 — pdbs 를 2 배로 하면 스파이크만 커지는 게 아니라 baseline 도 같이 커진다**: 실측에서 baseline 이 30.7 → 63.2 GB 로 따라 올라가 남은 여유는 14.10 GiB 였고, 요구는 24.49 GiB 였다. 헤드룸은 반드시 **바꾼 뒤의 설정**으로 재야 한다. step 20 근처에서 메모리가 평평해 보이는 것도 근거가 못 된다 — 수렴이 아니라 그 구간 샘플이 짧았을 뿐이고, 데이터 max 는 p99 의 2.5 배다. 정본은 `scripts/gpu_policy.py::_FORCE_HALF_BATCH_DATASETS`, 회귀 방지는 `tests/test_gpu_policy.py::test_exp08_force_half_batch_is_not_removable_by_headroom_argument`.
