...
현재 기준선은 **911 passed** 다(2026-08-29). 이 수가 줄면 회귀로 본다 — 단 **테스트를 의도적으로 삭제한 변경은 예외**이고, 그때는 삭제 개수까지 세어 새 기준선을 여기에 갱신한다(861 + M1a 이식 49 + StrEnum 대체 검증 1 = 911).

> 기준선 이력: 869 − canvas 17 + converter dedup 6 = 858(2026-07-22 기재) → 실측 861 → 911.
> **858 과 861 의 3 차이는 회귀가 아니라 기재 누락이다.** 2026-08-29 실측 시점에 `.venv` 의
> editable 설치가 리포 이전 경로(`~/Desktop/Projects/...`)를 가리켜 스위트가 **아예 실행되지 않는**
> 상태였고, `uv sync --extra dev` 로 복구한 뒤 861 을 실측했다. 같은 증상(`ModuleNotFoundError:
> No module named 'monkey_collector'`)이 보이면 코드가 아니라 venv 를 먼저 의심하라.
