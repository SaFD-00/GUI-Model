# Monkey-Collector - GUI World Modeling Data Collection Pipeline

Monkey-Collector 프로젝트의 Phase 1 Stage 1 (GUI World Modeling) 학습을 위한 자동 데이터 수집 파이프라인.

Android Virtual Device(AVD)에서 **Smart Explorer**(XML 기반 지능형 액션 선택)로 UI 이벤트를 생성하고, AccessibilityService 앱이 UI 변화를 감지하여 스크린샷 + XML을 서버로 전송한다. 수집된 raw data는 annotation pipeline을 통해 grounding/OCR/state_diff/element_qa/world_modeling 학습 데이터로 자동 변환된다.
...
### Smart Explorer vs Random Monkey

| 항목 | Random Monkey (`adb shell monkey`) | Smart Explorer |
|------|--------------------------------------|-------------|
...
1. Server → Smart Explorer가 앱 실행 후 초기 XML 획득
2. Smart Explorer → XML 파싱 → clickable/editable/scrollable 요소 식별 → 가중치 기반 액션 선택
...
```bash
# 단일 앱 수집 (Smart Explorer 100 steps)
...
```bash
# Calculator 앱에서 Smart Explorer 200 steps
...
### Smart Explorer 가중치 동적 조정

Smart Explorer는 현재 화면의 UI 요소 유형에 따라 액션 가중치를 자동 조정한다:
...
│   ├── cli.py                      # CLI 엔트리포인트
│   ├── orchestrator.py             # 수집 오케스트레이터 (Smart Explorer + TCP 서버 조율)
│   ├── server.py                   # TCP 서버 (Android 앱에서 데이터 수신 + XML 동기화)
│   ├── explorer/
│   │   ├── smart_explorer.py         # Smart Explorer (XML 기반 지능형 액션 선택)
...
```json
{
  "id": "mc_{session}_{step}_{task_type}_{idx}",
...
Smart Explorer 또는 앱 자체가 타겟 앱 외부로 벗어나는 것을 다층 방어한다.
...
| MobileForge `smart_explorer.py` | `explorer/smart_explorer.py` | 액션 선택 로직 적응 포팅 |
