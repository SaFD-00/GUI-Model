...
여러 AVD 를 사용해 `apps.csv` 기반으로 **카테고리별 / 우선순위별** 데이터 수집을 순차 수행한다. 내부 동작은 한 번에 AVD 한 대만 부팅 → 해당 AVD 에 배분된 job 전부 수집 → 종료 후 다음 AVD 로 이동하는 방식이다. job 은 AVD 수 기준 round-robin 으로 사전 배분된다.

**Resume 기본 동작**: `data/raw/{category}/{package}/metadata.json` 의 `completed_at` 이 채워진 앱은 자동으로 건너뛴다 (중단된 세션은 `Collector` 의 per-session resume 로직으로 이어서 수집). 완료 상태를 무시하고 전부 다시 수집하려면 `--force` 를 사용하고, 데이터 자체를 지우려면 `reset` 서브커맨드를 사용한다.
...
3. **Collector 앱 빌드·설치** — `app/` Android 앱을 빌드한 뒤 각 AVD 에 설치한다. `scripts/install_collector_to_avds.sh` 로 단일 AVD 에 설치할 수 있다 (여러 AVD 면 AVD 마다 한 번씩 호출).
...
- `--avds`: 사전 생성된 AVD 이름의 콤마 구분 리스트 (필수). job 은 이 순서대로 round-robin 배분되며 한 번에 한 AVD 만 부팅된다.
...
- `--force`: `completed_at` 이 채워진 앱도 다시 수집 (resume-skip 우회)
...
수집된 세션 데이터를 범위 단위로 삭제한다. `sweep` 의 resume-skip 을 되돌리거나, 카테고리/패키지 단위로 깔끔하게 재시작할 때 사용한다.

```bash
# 전체 삭제
monkey-collect reset --all --yes

# 특정 카테고리 전체 삭제 (실행 전 확인 프롬프트)
monkey-collect reset --categories Shopping,Productivity

# 특정 패키지만 (모든 카테고리에서 검색)
monkey-collect reset --packages com.example.foo,com.example.bar --yes

# 카테고리 × 패키지 교집합
monkey-collect reset --categories Shopping --packages com.example.foo --yes

# apps.csv + priority 필터로 대상 해소
monkey-collect reset --apps-csv apps.csv --priorities High --yes

# 미리 보기
monkey-collect reset --categories Shopping --dry-run
```

주요 옵션:

- `--all`: `--output` 전체를 삭제 (다른 스코프 플래그와 상호 배타)
- `--categories`: 삭제할 카테고리 콤마 구분 리스트
- `--packages`: 삭제할 package_id 리스트. `--categories` 와 함께 쓰면 교집합
- `--apps-csv`: apps.csv 를 `--categories` / `--priorities` 로 필터해서 대상 (cat, pkg) 해소
...
단일 AVD 를 headless 로 부팅 (`-no-window -no-audio -no-boot-anim`) → `adb install -r` → `adb emu kill` 까지 한 번에 처리한다. 여러 AVD 가 필요하면 AVD 마다 한 번씩 호출한다.
...
bash scripts/install_collector_to_avds.sh \
  ...
  monkey-1
# 로그: /tmp/monkey-install-logs/monkey-1.log(.emu|.install)
```

설치 또는 검증 실패 시 종료 코드는 1.
