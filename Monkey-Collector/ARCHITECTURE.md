...
└──────────────────────────────────────────┘      │  infra/xml/ (UITree + StructuredXmlParser) │
                                                  └───────────────────────────────────────┘
```
...
#### CollectionServer (`server/infra/network/server.py`)
...
#### Collector (`server/pipeline/collector.py` + `session_manager.py` + `collection_loop.py` + `recovery.py`)
...
#### SmartExplorer (`server/pipeline/explorer.py`)
...
#### AdbClient (`server/infra/device/adb.py`)

ADB 명령어 래퍼. `PATH`, `ANDROID_HOME`, `~/Library/Android/sdk` 순서로 자동 탐색.

주요 메서드: `tap`, `swipe`, `input_text` (특수 문자 escaping), `long_press`, `press_back`, `press_home`, `launch_app` (Java inner class `$` 이스케이프), `get_current_package`, `get_current_activity`, `get_declared_activities`, `get_device_resolution`

#### DataWriter (`server/infra/storage/storage.py`)
...
#### UITree / UIElement (`server/infra/xml/ui_tree.py`)
...
#### StructuredXmlParser (`server/infra/xml/parser/`)
...
#### Converter (`server/export/converter.py`)
