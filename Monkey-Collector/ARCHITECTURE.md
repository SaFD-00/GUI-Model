...
- `_run_session()`: 단일 세션 실행 (서버 생명주기는 호출자가 관리). **같은 앱의 기존 세션을 자동 감지하여 resume** — `DataWriter.find_existing_session()`으로 검색, 있으면 `resume_session()`으로 step 이어서 시작, tracker들도 `resume()`로 CSV append. `--new-session` 플래그 시 항상 새 디렉토리 생성. 세션 종료 시 전체 XML로 PageGraph 재빌드 (`build_graph_from_session()`)
...
**세션 이어서 저장 (Resume)**: 같은 앱 패키지에 대한 기존 세션이 있으면 새 디렉토리를 만들지 않고 기존 세션에 이어서 저장한다.
- `find_existing_session(package)`: `{package}_*` 패턴으로 최신 기존 세션 검색
...
Entry point: `monkey-collect`. Subcommands: `run`, `convert`, `convert-all`, `page-map`, `page-map-all`. `run`은 기본 다중 세션 모드, `--single` 플래그로 단일 세션 모드 전환. `--new-session` 플래그로 같은 앱이라도 새 세션 디렉토리 강제 생성 (기본: 기존 세션에 이어서 저장).
...
- **How**: `_run_session()` 시작 시 `DataWriter.find_existing_session(package)`로 같은 앱의 최신 기존 세션 검색. 있으면 `resume_session()`으로 step_count 복원 (XML 파일 수 기반), tracker들 `resume()`으로 기존 CSV에서 상태 복원 후 append. 세션 종료 시 `build_graph_from_session()`으로 전체 XML 재빌드하여 통합 PageGraph 생성. `--new-session` 플래그로 강제 새 세션 가능
